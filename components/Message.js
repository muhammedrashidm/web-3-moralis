...
function Message({ message }) {
    ...
    return (
        <div className={`flex items-end space-x-2 relative ${isUserMessage && 'justify-end'}`}>
            ...
            <TimeAgo
                className={`text-[10px] italic text-gray-500   ${isUserMessage && 'order-first pr-1'}`}
                datetime={message.createdAt} />
            <p className={`absolute -bottom-5 text-xs ${isUserMessage ? 'text-pink-400' : 'text-blue-400'}`}>{message.get("username")}</p>
        </div>
    )
}
